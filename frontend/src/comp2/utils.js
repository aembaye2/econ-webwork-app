...
export const handleGeneratePDF = async (
  ...
  userInputData.forEach((question, index) => {
    let refName = question.refs[3];
    //let imgSrc = `/${question.refs[1]}`; //let imgSrc = process.env.PUBLIC_URL + "/" + question.refs[1];
    let imgSrc = new URL(`../assets/${question.refs[1]}`, import.meta.url).href;
    if (refName) {
